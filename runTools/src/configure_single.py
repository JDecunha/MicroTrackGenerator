...
def generate_runfile(particle,energy,macro,walltime,jobname,jobdir,templateString):
    ...
    # Render the template
    runfile_template_filled = templates.run_command_template.format(output_location = ("../output/" + particle + "/" + str(energy) + "MeV/" + particle),macro=macro,seed=str(random.randint(1,sys.maxint)))
